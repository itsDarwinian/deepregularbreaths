...
var percent = 0.15;
var blocks = document.getElementsByClassName("scroll-block");
...
function detector(event) {
  for (var i = 0; i < blocks.length; i++) {
    ...
    if (block.getBoundingClientRect().bottom <= window.innerHeight) {
      if (!block.classList.contains("pulled")) block.classList.add("pulled");
    } else {
      if (block.classList.contains("pulled")) block.classList.remove("pulled");
    }
  }
}
